...
with open(file, "r") as file:
    kol_vo_tovarov = int(file.readline())  # Читаем первую строку
    d = [line.split() for line in file]  # Разбиваем строки файла на элементы

# Преобразуем данные в список кортежей
items = [(int(article), int(price), int(status)) for article, price, status in d]

# Вычисляем общую стоимость и среднюю цену
full_price = sum(item[1] for item in items)
average_price = full_price / kol_vo_tovarov  # Средняя цена

# Отбираем только дорогие товары
expensive_items = [item for item in items if item[1] > average_price]

# Подсчёт проданных и оставшихся товаров
sales = {}
for article, price, status in expensive_items:
    if article not in sales:
        sales[article] = [0, 0, price]  # [продано, осталось, цена]
...
sorted_dict = sorted(
    sales.items(),
    key=lambda f: (-f[1][0], -f[1][2], f[1][1])  # Сортировка по проданным, цене, оставшимся
)

# Лидер продаж
leader_article, (sold, remaining, price) = sorted_dict[0]

# Суммарная стоимость оставшихся единиц
total_value = remaining * price
